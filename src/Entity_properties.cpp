...
struct Entity
{
  ...
  Entity(float _x = 0, float _y = 0, float Width = 0, float Height = 0, const char* vertex_shader = 0, const char* fragement_shader = 0)
  {
	...
	FILE* vertex_shader_file = fopen(vertex_shader, "r");
	fseek(vertex_shader_file, 0, SEEK_END);
	int length = ftell(vertex_shader_file);
	fseek(vertex_shader_file, 0, SEEK_SET);
	vertex_shader_source = (char*)malloc(length);
	fread(vertex_shader_source, length, 1, vertex_shader_file);

	FILE* fragment_shader_file = fopen(fragement_shader, "r");
	fseek(fragment_shader_file, 0, SEEK_END);
	length = ftell(fragment_shader_file);
	fseek(fragment_shader_file, 0, SEEK_SET);
    fragment_shader_source = (char*)malloc(length);
	fread(fragment_shader_source, length, 1, fragment_shader_file);
	...
	glShaderSource(fragement_shader_ID, 1, &fragment_shader_source, 0);
